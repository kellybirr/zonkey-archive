...
namespace Zonkey.ObjectModel
{
	...
	{
		private readonly static Dictionary<Type, Func<object>> _typeRegistry = new Dictionary<Type, Func<object>>();
        ...
        public static Func<T> CompileDefaultFactory<T>(Type type)
	    {
            ...
            if (ctor == null)
                throw new InvalidOperationException(string.Format("Type '{0}' does not have a default constructor", type.FullName));
