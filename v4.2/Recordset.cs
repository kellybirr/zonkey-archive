...
{
	/// <summary>
	/// A partial emulator of an ADO recordset object
	/// </summary>
	public class Recordset : IDisposable
	{
		...
		public Recordset(string connectionName) : this(DbConnectionFactory.OpenConnection(connectionName))
		{ }
		...
		/// <returns></returns>
		public int Open(string sql, params object[] parameters)
		{
		...
		/// <param name="parameters">The parameters.</param>
		/// <returns></returns>
		public int Open(string sql, CommandType commandType, params object[] parameters)
		{
			// check valid state
			if ( (Connection == null) || (Connection.State != ConnectionState.Open))
				throw new InvalidOperationException("Must provide an open Connection before opening Recordset");
			...
			dm.FillDataTable(_dt, sql, commandType, parameters);
		...
		{
			get { return (_dt == null) ? -1 : _dt.Rows.Count; }
		}
...
// ReSharper restore LoopCanBeConvertedToQuery
		...
		public int UpdateBatch()
		{
		    ...
		    {
		        CreateCommandBuilder = (t, c) => new DataTableCommandBuilder(t, c) {UseQuotedIdentifier = true}
		    };

            // do the update
            return dta.SaveChanges(_dt);
		}
