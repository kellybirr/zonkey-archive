...
namespace Zonkey
{
    ...
    public static class DbConnectionFactory
    {
        ...
        {
			return CreateConnection(name, (ConnectionStringSettings)null);
        }

		/// <summary>
		/// Creates, but does not open a new connection configured based on a named ConnectionString in the App.Config/Web.Config or supplied additionalConnectionStrings
		/// </summary>
		/// <param name="name">the name of the ConnectionString element from the config file</param>
		/// <param name="overrideConnectionString">User-supplied override connection string.</param>
		/// <returns>an open DbConnection</returns>
		public static DbConnection CreateConnection(string name, ConnectionStringSettings overrideConnectionString)
		{
			if (string.IsNullOrEmpty(name))
				throw new ArgumentNullException("name");

			ConnectionStringSettings settings = overrideConnectionString ?? ConfigurationManager.ConnectionStrings[name];

			if (settings == null)
				throw new InvalidOperationException(string.Format("A ConnectionString element named '{0}' was not found in the App.Config/Web.Config file or in the AdditionalConnectionString cache", name));
		    ...
		    string procesedConnStr = ConfigEnv.ProcessString(settings.ConnectionString);
			return CreateConnection(settings.ProviderName, procesedConnStr);
		}
        ...
        public static DbConnection CreateConnection(string providerName, string connectionString)
        {
            ...
            // shortcut for built-in MockDbConnection
            if (providerName == "Zonkey.Mocks.MockDbConnection")
                return new Mocks.MockDbConnection(connectionString);
