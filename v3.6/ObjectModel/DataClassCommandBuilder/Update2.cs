...
namespace Zonkey.ObjectModel
{
    ...
    public partial class DataClassCommandBuilder
    {
        ...
        public DbCommand[] GetUpdate2Commands(ISavable obj, UpdateCriteria criteria, bool doSelectBack)
        {
            ...
            // intit command array
            var commands = new DbCommand[2];
            commands[0] = GetTextCommand("");
            commands[1] = (doSelectBack) ? GetTextCommand("") : null;
            ...
            foreach (KeyValuePair<string, object> changedField in obj.OriginalValues)
            {
                    ...
                    )
                {
                    ...
                    {
                        DbParameter whereParam = CreateWhereParam(commands[0], field);
                ...
                DbParameter setParm = CreateSetParam(commands[0], field);
            ...
            foreach (IDataMapField field in _dataMap.KeyFields)
            {
                ...
                if (doSelectBack)
                {
                    DbParameter keyParam = CreateWhereParam(commands[1], field);
            ...
            commands[0].CommandText = String.Format("UPDATE {0} SET {1} WHERE {2};", SaveToTable, setString, whereString);
            commands[0].Parameters.AddRange(setParmList.ToArray());
            commands[0].Parameters.AddRange(whereParmList.ToArray());

            if (doSelectBack)
            {
                commands[1].CommandText = String.Format("SELECT {0} FROM {1} WHERE {2};", ColumnsString, TableName, keyString);
                commands[1].Parameters.AddRange(keyParmList.ToArray());
            }
