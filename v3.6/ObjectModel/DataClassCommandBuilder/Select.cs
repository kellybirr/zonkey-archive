...
namespace Zonkey.ObjectModel
{
    ...
    public partial class DataClassCommandBuilder
    {
        ...
        public DbCommand SelectByKeysCommand
        {
            get
            {
                if (_selectByKeysCommand == null)
                {
                    ...
                    _selectByKeysCommand.CommandText = string.Format("SELECT {0} FROM {1} WHERE {2}", ColumnsString, TableName, whereString);
                }
        ...
        {
            var sql = _dialect.FormatLimitQuery(ColumnsString, TableName, filter, sort, start, length);
        ...
        {            
            ...
            command.CommandText = _dialect.FormatLimitQuery(ColumnsString, TableName, whereString, sort, start, length);
            return command;
        }
        ...
        {
            var commandText = new StringBuilder();
            commandText.AppendFormat("SELECT COUNT(*) AS ZONKEY_ROWCOUNT FROM {0}", TableName);
        ...
        public DbCommand GetRequeryCommand(object obj)
        {
            ...
            command.CommandText = string.Format("SELECT {0} FROM {1} WHERE {2}; ", selectString, TableName, whereString);
