...
namespace Zonkey.Text
{
    ...
    public sealed class CsvReader : IDataReader
    {
        ...
        private readonly StringCollection _record = new StringCollection();
        ...
        public bool Read()
        {
        	...
        	{
				_lineNum++;
				szLine = BaseReader.ReadLine();
				if (szLine == null) return false;	
        	} while ((LineFilter != null) && (! LineFilter(_lineNum, szLine)));

        	_currentLineLength = szLine.Trim().Length;
            if (_currentLineLength == 0) return false;

            bool InQuote = false;
            var sbItem = new StringBuilder();

            for (int n = 0; n < szLine.Length; n++)
            {
                char c = szLine[n];
                ...
                {
                    int j = n + 1, k = n + 2;
                    if ((szLine.Length > k) && (szLine[j] == _textQualifier) && (szLine[k] != _delimiter))
                    {
                        sbItem.Append(c);
                        n++;
                    }
        ...
        public string this[int index, int maxLength]
        {
            get 
            {
                if (maxLength < _record[index].Length)
                    return _record[index].Substring(0, maxLength);
                else
        ...
        /// </summary>
        /// <value></value>
        object System.Data.IDataRecord.this[int index]
        {
            get { return _record[index]; }
        }
        ...
        public int GetValues(object[] values)
        {
            ...
            string[] saTemp = new string[_record.Count];
        ...
        public long GetChars(int i, long fieldoffset, char[] buffer, int bufferoffset, int length)
        {
            ...
            Array.Copy(ch, fieldoffset, buffer, bufferoffset, len);
            return len;
        }
