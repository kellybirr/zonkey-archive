    ...
    public partial class DataClassAdapter<T>
    {
        ...
        public bool DeleteItem(T obj)
        {
            ...
            if (_deleteItemCommand == null)
                _deleteItemCommand = CommandBuilder.DeleteItemCommand;

            for (int i = 0; i < keyValues.Count; i++)
                _deleteItemCommand.Parameters[i].Value = keyValues[i];

            try
            {
                int result = ExecuteNonQueryInternal(_deleteItemCommand);
                return (result == 1);
            }
            catch (Exception ex)
            {
